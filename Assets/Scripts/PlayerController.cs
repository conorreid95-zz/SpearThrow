...
public class PlayerController : MonoBehaviour
{
    ...
    void CheckPastLine()
    {
        if(transform.position.x < 41.09f && !pastLine)
        {
            pastLine = true;
            white_Material.color = Color.red;
            Invoke("StartFaultProcess", 2f);
        }
    }

    void StartFaultProcess()
    {
        gameController.GetComponent<GameController>().ProcessNewJavelinScore(0f);
    }
        ...
        else
        {
            if ((Input.GetKeyUp(KeyCode.Space) || Input.GetKeyUp(KeyCode.Mouse0)) || (!spearReleased && (spear.transform.rotation.eulerAngles.z < 1f))) //if throw is released
            {
                if (!spearReleased)
                {
                    ...
                    normalisedVelocity = normalisedVelocity * (8.25f + UnityEngine.Random.Range(0.5f, 1f)); //multiply to get a force to add onto throw
